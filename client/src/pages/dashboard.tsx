                    ...
                    {chapters.slice(0, 6).map((chapter) => {
                      ...
                      return (
                        <div key={chapter.id} 
                          ...
                          className="chapter-card p-4 border rounded-lg bg-white hover:bg-blue-50 hover:border-blue-200 transition-colors cursor-pointer group"
                        >
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              {completed ? (
                                <div className="flex items-center justify-center w-8 h-8 bg-green-500 text-white rounded-full text-xs font-bold">
                                  ✓
                                </div>
                              ) : (
                                <div className="flex items-center justify-center w-8 h-8 bg-literary-blue text-white rounded-full text-xs font-bold">
                                  {chapter.number}
                                </div>
                              )}
                              <div>
                                <p className="font-medium text-sm text-warm-gray group-hover:text-literary-blue transition-colors">
                                  {chapter.title}
                                </p>
                            ...
                            <div className="text-right">
                              <div className="text-sm font-medium text-literary-blue">{progress}%</div>
                              <div className="w-16 h-1 bg-gray-200 rounded-full mt-1">
                                <div 
                                  className={`h-full rounded-full transition-all ${completed ? 'bg-green-500' : 'bg-literary-blue'}`}
                                  style={{ width: `${progress}%` }}
                                ></div>
                              </div>
                            </div>
